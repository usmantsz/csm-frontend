...
const TeamMemberLogin = () => {
    ...
    const [rememberMe, setRememberMe] = useState(() => localStorage.getItem('remember_team') === 'true');

    useEffect(() => {
        const saved = localStorage.getItem('remember_team_cnic');
        if (saved) {
    ...
    const submitForm = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if (response.data.token && response.data.data) {
                ...
                // Team Member login: accept only Sub Admin (2) or Team Member (3)
                if (userRole === 0) {
                    Notification({
                ...
                if (userRole !== 2 && userRole !== 3) {
                    Notification({
                        text: 'Only Sub Admin or Team Member can login here.',
                        color: 'warning',
                    });
                    return;
                }
                ...
                if (rememberMe) {
                    localStorage.setItem('remember_team', 'true');
                    localStorage.setItem('remember_team_cnic', String(data.userEmail || '').replace(/\D/g, '').slice(0, 13));
                } else {
                    localStorage.removeItem('remember_team');
                    localStorage.removeItem('remember_team_cnic');
                }
        ...
        <div className="h-screen overflow-hidden relative flex items-center justify-center px-4 py-4">
            ...
            <div className="auth-page-shell relative w-full max-w-md max-h-full overflow-y-auto">
                <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden border border-white/20 dark:border-gray-700/50">
                    <div className="auth-login-card-header relative flex flex-col items-center bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-700 dark:from-emerald-800 dark:to-teal-900 overflow-hidden">
                        ...
                        <div className="relative auth-page-logo rounded-2xl bg-white/25 backdrop-blur-sm ring-4 ring-white/30 flex items-center justify-center">
                            <img src="/assets/images/commission-shop-logo.png" alt="Commission Shop" className="w-full h-full object-contain p-2" />
                        </div>
                        <h1 className="relative auth-page-title text-white drop-shadow-sm">Sign In</h1>
                        <p className="relative auth-page-subtitle text-emerald-100 mt-1">Sign in with CNIC</p>
                    </div>
                    <div className="auth-login-card-body">
                        <form onSubmit={submitForm} className="space-y-5">
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className={`form-input pl-10 w-full rounded-lg border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-emerald-500/30 ${errors.userEmail ? 'border-danger focus:ring-danger/30' : ''}`}
                                        disabled={loading}
                                    />
                                </div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className={`form-input pl-10 pr-10 w-full rounded-lg border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-emerald-500/30 ${errors.userPassword ? 'border-danger focus:ring-danger/30' : ''}`}
                                        disabled={loading}
                                    />
                                    <button
                                        type="button"
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <Link to="/forgotpassword" className="text-sm font-medium text-emerald-600 dark:text-emerald-400 hover:underline">
                                    Forgot password?
                                </Link>
                            </div>
                            <button
                                type="submit"
                                disabled={loading}
                                className="auth-page-btn w-full flex justify-center items-center py-3 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 focus:ring-2 focus:ring-emerald-500/50 shadow-lg shadow-emerald-900/20 disabled:opacity-50 transition-all duration-200"
                            >
                                {loading ? (
